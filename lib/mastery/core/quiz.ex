defmodule Mastery.Core.Quiz do
  ...
  defp add_template_to_field(quiz, field) do
    template = template(quiz)
    updated_field = Map.get(quiz, field) ++ [template]
    Map.put(quiz, field, updated_field)
  end

  defp reset_template_cycle(%{templates: templates, used: used } = quiz) when map_size(templates) == 0 do
      %__MODULE__{quiz | templates: Enum.group_by(used, & &1.category), used: []}
  end

  defp reset_template_cycle(quiz), do: quiz

  def anwer_question(quiz, %Response{correct: true} = response) do
    new_quiz =
      quiz
      |> inc_record
